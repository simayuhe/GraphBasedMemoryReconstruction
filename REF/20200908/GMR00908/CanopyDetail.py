...
class Canopy:
    ...
    def euclideanDistanceT(self, vec1, vec2):
        #print("vec1",vec1)
        # 输入是两个tensor
        dist =math.sqrt(tf.reduce_sum((vec1 - vec2) ** 2))
        return dist
    ...
    def clustering(self,dataset):
        self.dataset =dataset# 这个dataset 有索引和特征组成的字典
        ...
        if self.t1 == 0:
            ...
            while len(self.dataset) != 0:
                ...
                rand_index = random.choice(list(self.dataset))
                #print("rand index",rand_index)
                # 随机获取一个中心点，定为P点
                current_center = self.dataset[rand_index]
                #print("currentcenter",current_center)
                # 初始化P点的canopy类容器
                current_center_list = [] 
                # 初始化P点的删除容器
                delete_list = [] 
               ...
               # print("------------------len before del",len(self.dataset))
                del self.dataset[rand_index]
               # print("------------------len after del",len(self.dataset))
                for key,datum in self.dataset.items():
                    #datum = self.dataset[datum_j]
                    # 计算选取的中心点P到每个点之间的距离
                    #distance = self.euclideanDistance(current_center, datum)
                    distance = self.euclideanDistanceT(current_center, datum)
                    #print(distance)
                    #print(distance)
                    if distance < self.t1:
                        # 若距离小于t1，则将点归入P点的canopy类
                        current_center_list.append(key)
                    if distance < self.t2:
                        # 若小于t2则归入删除容器
                        delete_list.append(key)
                [self.dataset.pop(k) for k in delete_list]
                #self.dataset = np.delete(self.dataset, delete_list, 0)
                canopies.append((rand_index, current_center_list))
            return canopies
...
def main():
    ...
    y_pred = KMeans(n_clusters=len(canopies), random_state=random_state).fit_predict(X)
